	...
	<view class="container-all">
		...
		<uni-table  stripe   border="false" emptyText="暂无更多数据" >
			...
			<uni-tr v-for="(item,index) in context  " :key='index'>
				<uni-td  align="center"><image :src="item.imgurl+item.imgsuffix" @click="clickImg(item)"></image></uni-td>
	...
	export default {
		...
		methods: {
			...
			edit(items) {
				...
				uni.navigateTo({
						url: "/pages/modify/modify?name=" + items.name + "&detail=" + items.detail + "&url=" + items
							.imgurl+"&phone=" + items.phone+"&ename=" + items.ename+"&imgsuffix=" + items.imgsuffix
					}),
					console.log(name)
			},
			onSearch() {
				if (this.searchname) {
					uni.request({
						...
						success: res => {
							...
							console.log("this.context--->", this.context[0].imgsuffix)
			...
			clickImg(item) {
				...
				uni.previewImage({
					urls: [item.imgurl+item.imgsuffix], //需要预览的图片http链接列表，多张的时候，url直接写在后面就行了
